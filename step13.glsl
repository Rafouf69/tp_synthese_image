...
float terrain(vec3 p)
{
    ...
    float eau1 = turbulence(p.xz, 0.2, cos(iTime*0.2)*0.2, 0.02, 3);
    float eau2 = turbulence(p.xz, sin(iTime*5.)*(-0.3), 0.05, 0.05, 2);
    float eau3 = turbulence(p.xz, 0.05, 0.1, cos(iTime)*0.9, 2);
    //float eau4 = turbulence(p.xz, cos(iTime*5.)*0.1, 0.1, cos(iTime)*0.3, 2);
    //float eau5 = turbulence(p.xz, 0.1, cos(iTime*2.)*0.1, cos(iTime)*0.2, 2);
    if(p.y > 0.1+cos(iTime)*0.02)
    {
...
// h : hit
// s : Number of steps
float Trace(vec3 o, vec3 u, out bool h,out int s, out bool w, out int stepsWater)
{
   h = false;	
    w =false;
   stepsWater = 0;
...
vec3 Shade(vec3 p, vec3 n, int s, vec3 rd)
{
   	...
   	float alpha = p.y-0.5;
    float turb = clamp(turbulence(p.xy,0.1,0.5,0.2,4),-0.2,0.2);  
    ...
    else if( p.y > 0.1 + cos(iTime)*0.02 && mod(p.y,0.12)<=0.01)
    {
    	c= vec3(-0.1,-0.1,-0.1);   
    }
	else if(p.y>0.1+cos(iTime)*0.02)
    {
	...
	else
    {
        ...
        c=vec3(0.0, 0.25, 0.4)+clamp(turbulence(p.xz, 0.2, cos(iTime*0.02)*0.2, 0.02, 3),-0.02,0.02);
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
   ...
   int s;
	int stepsWater;
   float t = Trace(ro, rd, hit,s, w, stepsWater);
   vec3 pos=ro+t*rd;
   // Shade background
