...
{
    float y = turbulence(p.xz, 0.71, 0.2, 0.38, 8);
    return y-p.y; 
}
// Transforms
...
vec3 Shade(vec3 p, vec3 n, int s, vec3 rd)
{
   ...
   vec3 l = normalize(rd - p);
    float turbTerre = turbulence(p.zy, 0.2, 0.7, 0.2, 80);
    float turbEau = turbulence(vec2(p.z,-5.0), 0.1, 0.1*iTime, 0.2, 8);
   // Not even Phong shading, use weighted cosine instead for smooth transitions
   float diff = 0.5*(1.0+dot(n, l));
    vec3 c;
    float py = conv(p.y);
	if( mod(p.y,0.07)<=0.0040 )
    {
       c=  vec3(0.0,0.0,0.0);
    }
    else
    { 
       c =  py*1.4/iResolution.x*vec3(0.7+turbTerre/2.0,0.5+turbTerre/2.0,0.2+turbTerre/2.0)+0.2*diff*Color;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
   ...
   if (hit)
   {
    ...
    float turbEau = turbulence(vec2(rd.y,-5.0), 0.1, 20.0+mod(iTime*0.1,2.0), 0.2, 8);
    float py = conv(pos.y);
	if(py<=-4.0)
    {
     	rgb= vec3(0.0, 0.25+turbEau/4.0,0.565+turbEau/4.0);   
    }
