...
vec3 Shade(vec3 p, vec3 n, int s, vec3 rd, bool w)
{
    ...
    //vec3 lightPos = vec3( -19.8+4.0*mod(iTime,10.0),5.7-5.0*cos(0.6*mod(iTime,10.0)),-6.7);
   	vec3 lightColor = vec3(sin(mod(iTime,10.0)*0.315), sin(mod(iTime,10.0)*0.315), sin(mod(iTime,10.0)*0.315));
